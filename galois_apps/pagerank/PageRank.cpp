...
struct PullAlgo {
  ...
  struct Initialize {
    ...
    Initialize(Graph& g): g(g) { }
    void operator()(Graph::GraphNode n) {
      LNode& data = g.getData(n, Galois::MethodFlag::ALL); //NONE
      data.value[0] = 1.0;
      data.value[1] = 1.0;
    }
  };
  ...
  struct Process {
    ...
    void operator()(const GNode& src) {
      LNode& sdata = graph.getData(src, Galois::MethodFlag::ALL); // NONE
      double sum = 0;

      for (auto jj = graph.edge_begin(src, Galois::MethodFlag::ALL), ej = graph.edge_end(src, Galois::MethodFlag::ALL); jj != ej; ++jj) {
        GNode dst = graph.getEdgeDst(jj);
        size_t num_neighbors = std::distance(graph.edge_begin(dst), graph.edge_end(dst)); // for fairness 
        float w = 1. / num_neighbors;  //graph.getEdgeData(jj);

        LNode& ddata = graph.getData(dst, Galois::MethodFlag::ALL);
  ...
  void operator()(Graph& graph) {
    ...
    while (true) {
      ...
      Galois::for_each(graph.begin(), graph.end(), Process(this, graph, iteration));
      iteration += 1;
    ...
    if (iteration & 1) {
      ...
      Galois::do_all(graph.begin(), graph.end(), Copy(graph));
