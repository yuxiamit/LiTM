...
const char* name = "Maximal Matching";
const char* desc = "Computes a maximal matching of nodes in a graph";
const char* url = "maximal_matching";
...
struct SerialAlgo {
  ...
  bool findUnmatched(Graph& graph, GNode src) {
    ...
    for (Graph::edge_iterator ii = graph.edge_begin(src),
      ...
      if (dst != src && data.flag == UNMATCHED)
      {
        data.flag = MATCHED;
        me.flag = MATCHED;
        return true;
...
struct is_bad {
  ...
  bool operator()(GNode n) const {
    ...
    if (me.flag == MATCHED) {
      ...
      //return false;
      if(counter > 1) // it turns out galois will iterate a vertex multiple times.
      {
        std::cerr << "double matched edges:" << counter << "\n"; 
        return true;
      }
...
bool verify(Graph& graph) {
  //return true; // Galois serial seems to cause problems
